...
class Env():
    def __init__(self, agent_type, env_module_id=None):
        ...
        self.pub_result_per_step = rospy.Publisher('result_per_step', Float32MultiArray, queue_size=5)
        self.result_per_step = Float32MultiArray()
    ...
    def DispEpisodeCSVEachStep(self, reward, collision_count, goal_count, step_count, GoalRates, num_goals):
        self.ep_number = self.ep_number + 1
        log = {
            "ep_number": self.ep_number,
            "environment": self.respawn_goal.currentModuleName(),
        ...
        if len(GoalRates) == 0:
            self.result_per_step.data = [reward, self.current_time, float("%.4f" % 0), num_goals]
        else:
            self.result_per_step.data = [reward, self.current_time, float("%.4f" % GoalRates[-1]), num_goals]


        self.pub_result_per_step.publish(self.result_per_step)
